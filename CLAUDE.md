...
| `sphere.payments.sendWholeToken(request)` | `Promise<TransferResult>` | Move a COINLESS token whole (`{recipient, tokenId, memo?}`) |
...
- **Transfer** is `sendWholeToken({recipient, tokenId, memo?})`, a separate verb: a coin spend SELECTS
