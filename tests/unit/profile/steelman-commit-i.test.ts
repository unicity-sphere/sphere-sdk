...
describe('Commit I — steelman⁴ Nostr fetch fixes', () => {
  ...
  it('throws labeled error with reason=event_cap when MAX_EVENTS_PER_FETCH matching events arrive', async () => {
    ...
    const err = await fetchPromise.catch((e: Error) => e) as Error & { reason?: string };
    // event_cap fires when events.length reaches MAX_EVENTS_PER_FETCH (10k).
    // Verify we got either event_cap or dispatch_cap (both are correct
    // truncation signals for this flood).
    expect(['event_cap', 'dispatch_cap']).toContain(err.reason);
