...
supervise_faucet() {
    local backoff=5
    while true; do
        # Break out immediately if a SIGTERM/SIGINT has been received
        # by the parent. Without this check the loop happily relaunches
        # the faucet AFTER `docker stop` has fired its shutdown.
        if [ "$SHUTDOWN_REQUESTED" -eq 1 ]; then
            log "shutdown requested — supervisor exiting"
            return 0
        fi
        ...
        # Reset backoff if the faucet ran for more than 30 seconds —
        # a stable run shouldn't penalise the next restart.
        if [ $(( end_ts - start_ts )) -gt 30 ]; then
            backoff=5
        fi
        if [ "$SHUTDOWN_REQUESTED" -eq 1 ]; then
            return 0
        fi
        log "faucet exited after $(( end_ts - start_ts ))s; restarting in ${backoff}s"
        sleep "$backoff"
        backoff=$(( backoff * 2 ))
...
# Guard against nginx failing to launch (e.g. config template render
# error) before we reach the wait — `wait -n ""` is undefined behavior.
if [ -z "${NGINX_PID:-}" ]; then
    log "ERROR: nginx PID is empty — nginx failed to launch"
    handle_signal TERM
    exit 1
fi

# Wait for nginx to exit. nginx exiting is fatal; the supervisor
# loop above prevents faucet exits from reaching here.
wait -n "$NGINX_PID"
exit_code=$?
