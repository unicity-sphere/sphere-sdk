...
import {
  dispatchUxfInstantSendImpl,
  type DispatchInstantHost,
} from '../../extensions/uxf/pipeline/dispatch-instant';
...
import {
  writeOutbox as writeOutboxImpl,
  loadOutbox as loadOutboxImpl,
  saveToOutbox as saveToOutboxImpl,
  removeFromOutbox as removeFromOutboxImpl,
  type OutboxEntry,
  type OutboxOpsHost,
...
import {
  publishUxfBundle as publishUxfBundleImpl,
  type PublishUxfBundleHost,
  type PublishUxfBundleParams,
...
import {
  dispatchUxfConservativeSendImpl,
  type DispatchConservativeHost,
} from '../../extensions/uxf/pipeline/dispatch-conservative';
import {
...
// surface after the Phase 5 [B] extraction.
export {
  buildDefaultFinalizationWorkerSender,
  buildDefaultFinalizationWorkerRecipient,
  buildDefaultInclusionProofImporter,
...
import {
  ...
  scheduleV6RecoverPermanentSaveRetryImpl,
  stopV6RecoverPermanentSaveRetryImpl,
  saveV6RecoverPermanentImpl,
  restoreV6RecoverPermanentImpl,
  applyV6RecoverPermanentInvalidStatusImpl,
  isV6RecoverPermanentTokenImpl,
  mintNametagImpl,
} from './legacy-v1';
...
export class PaymentsModule {
   ...
   */
  async publishUxfBundle(params: PublishUxfBundleParams): Promise<PublishUxfBundleResult> {
    return publishUxfBundleImpl(this.publishUxfBundleHost(), params);
  }

  /** Host-shim builder for {@link publishUxfBundleImpl}. */
  private publishUxfBundleHost(): PublishUxfBundleHost {
    return {
      ...
      resolveTransportPubkey: (recipient, peerInfo) =>
        this.resolveTransportPubkey(recipient, peerInfo),
    };
  }

  // NOTE: prior inline implementation of publishUxfBundle (~200 LoC)
  // moved verbatim to
  // extensions/uxf/pipeline/module-glue/publish-uxf-bundle.ts.
  ...
  private scheduleV6RecoverPermanentSaveRetry(): void {
    return scheduleV6RecoverPermanentSaveRetryImpl.call(this);
  }

  private stopV6RecoverPermanentSaveRetry(): void {
    return stopV6RecoverPermanentSaveRetryImpl.call(this);
  }
  ...
  async mintNametag(nametag: string): Promise<MintNametagResult> {
    return mintNametagImpl.call(this, nametag);
  }
  ...
  private async dispatchUxfConservativeSend(
    ...
    return dispatchUxfConservativeSendImpl(this.dispatchConservativeHost(), originalRequest);
  }

  /** Host-shim builder for {@link dispatchUxfConservativeSendImpl}. */
  private dispatchConservativeHost(): DispatchConservativeHost {
    return {
      tokens: this.tokens,
      parsedTokenCache: this.parsedTokenCache,
      spendPlanner: this.spendPlanner,
      spendQueue: this.spendQueue,
      ...
      resolveTransportPubkey: (recipient, peerInfo) =>
        this.resolveTransportPubkey(recipient, peerInfo),
      resolveRecipientAddress: (recipient, addressMode, peerInfo) =>
        this.resolveRecipientAddress(recipient, addressMode, peerInfo),
      resolveCoinIdSymbol: (request) => this.resolveCoinIdSymbol(request),
      createSigningService: () => this.createSigningService(),
      createSdkCommitment: (token, recipientAddress, signingService, onChainMessage) =>
        this.createSdkCommitment(token, recipientAddress, signingService, onChainMessage),
      submitCommitmentClassified: (stClient, oracle, commitment, classify) =>
        this.submitCommitmentClassified(stClient, oracle, commitment, classify),
      assertNoDuplicateBundleMembership: (candidateTokenIds, options) =>
        this.assertNoDuplicateBundleMembership(candidateTokenIds, options),
      save: () => this.save(),
      addToken: (token) => this.addToken(token),
      removeToken: (tokenId, excludeReservationId) =>
        this.removeToken(tokenId, excludeReservationId),
      saveToOutbox: (transfer, recipient) => this.saveToOutbox(transfer, recipient),
      removeFromOutbox: (transferId) => this.removeFromOutbox(transferId),
      writeSentEntryFromOutbox: (entry, opLabel) =>
        this.writeSentEntryFromOutbox(entry, opLabel),
      extractCoinAmountForCache: (sdkToken, coinIdHex) =>
        this.extractCoinAmountForCache(sdkToken, coinIdHex),
      recordUxfBundleSentHistory: (args) => this.recordUxfBundleSentHistory(args),
      emitDoubleSpendDetectedIfApplicable: (err) =>
        this.emitDoubleSpendDetectedIfApplicable(err),
      getCoinSymbol: (coinId) => this.getCoinSymbol(coinId),
      getCoinName: (coinId) => this.getCoinName(coinId),
      getCoinDecimals: (coinId) => this.getCoinDecimals(coinId),
      getCoinIconUrl: (coinId) => this.getCoinIconUrl(coinId),
    };
  }
  ...
  private async dispatchUxfInstantSend(
    ...
    return dispatchUxfInstantSendImpl(this.dispatchInstantHost(), originalRequest);
  }

  /**
   * Host-shim builder for {@link dispatchUxfInstantSendImpl}. Snapshotted
   * per call so mutable containers (`tokens`, `_senderOutboxMap`, etc.)
   * are exposed by direct reference for in-place edits, while nullable
   * install slots (`_outboxWriter`, `finalizationWorkerSender`) capture
   * the current values at call time — install methods do not fire
   * mid-send, so the snapshot preserves the original `this.foo` read
   * semantics.
   */
  private dispatchInstantHost(): DispatchInstantHost {
    return {
      deps: this.deps!,
      resolveCoinIdSymbol: (req) => this.resolveCoinIdSymbol(req),
      resolveTransportPubkey: (recipient, peerInfo) =>
        this.resolveTransportPubkey(recipient, peerInfo),
      resolveRecipientAddress: (recipient, addressMode, peerInfo) =>
        this.resolveRecipientAddress(recipient, addressMode, peerInfo),
      createSigningService: () => this.createSigningService(),
      assertNoDuplicateBundleMembership: (ids, options) =>
        this.assertNoDuplicateBundleMembership(ids, options),
      save: () => this.save(),
      addToken: (token) => this.addToken(token),
      getCoinSymbol: (coinId) => this.getCoinSymbol(coinId),
      getCoinName: (coinId) => this.getCoinName(coinId),
      ...
      senderRequestContextMap: this._senderRequestContextMap,
      senderOutboxMap: this._senderOutboxMap,
      pendingBackgroundTasks: this.pendingBackgroundTasks,
      outboxWriter: this._outboxWriter,
      finalizationWorkerSender: this.finalizationWorkerSender,
    };
  }
  ...
  /**
   * Build the OUTBOX ops host shim — snapshotted per call so mutations
   * to `_lastPinnedOutbox*` still land on the facade.
   *
   * Delegates to `extensions/uxf/pipeline/module-glue/outbox-ops.ts`
   * (see the Phase 5 disposition ledger). Behavior preserved verbatim.
   */
  private outboxOpsHost(): OutboxOpsHost {
    return {
      storage: this.deps!.storage,
      cidRefStore: this.deps!.cidRefStore ?? null,
      getLastPinned: () => ({
        json: this._lastPinnedOutboxJson,
        ref: this._lastPinnedOutboxRef,
      }),
      setLastPinned: (next) => {
        this._lastPinnedOutboxJson = next.json;
        this._lastPinnedOutboxRef = next.ref;
      },
      setStorageEntry: (key, value, entryType: OutboxSetEntryType) =>
        this.setStorageEntry(key, value, entryType),
    };
  }

  private async saveToOutbox(transfer: TransferResult, recipient: string): Promise<void> {
    return this.enqueueOutboxOp(() => saveToOutboxImpl(this.outboxOpsHost(), transfer, recipient));
  }

  private async removeFromOutbox(transferId: string): Promise<void> {
    return this.enqueueOutboxOp(() => removeFromOutboxImpl(this.outboxOpsHost(), transferId));
  }
   ...
   * (see the Phase 5 disposition ledger). Behavior preserved verbatim.
   */
  private async writeOutbox(list: OutboxEntry[]): Promise<void> {
    return writeOutboxImpl(this.outboxOpsHost(), list);
  }

  /**
   * Load the outbox — dual-read per PROFILE-CID-REFERENCES.md §6.
   *
   * Delegates to `extensions/uxf/pipeline/module-glue/outbox-ops.ts`
   * (see the Phase 5 disposition ledger). Behavior preserved verbatim.
   */
  private async loadOutbox(): Promise<OutboxEntry[]> {
    return loadOutboxImpl(this.outboxOpsHost());
  }
  ...
  private async saveV6RecoverPermanent(): Promise<void> {
    return saveV6RecoverPermanentImpl.call(this);
  }
  ...
  private async restoreV6RecoverPermanent(): Promise<void> {
    return restoreV6RecoverPermanentImpl.call(this);
  }
  ...
  private applyV6RecoverPermanentInvalidStatus(): number {
    return applyV6RecoverPermanentInvalidStatusImpl.call(this);
  }
  ...
  private isV6RecoverPermanentToken(token: Token, mapKey?: string): boolean {
    return isV6RecoverPermanentTokenImpl.call(this, token, mapKey);
  }
