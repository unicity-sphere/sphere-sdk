...
Phase 2 work items 7 (orphan sweeper disambiguation — distinguish multi-device double-spend from crash-window orphan) and 8 (`getAssets`/balance regression test) remain open per the original split — both forensic / observability surfaces, not correctness paths now that work item 5 closes the `unconfirmedAmount` inflation.
