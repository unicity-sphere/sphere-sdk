...
**Known follow-up (latent bug — separate item):** under D.1b/D.2/E the aggregator pointer now carries a *snapshot* CID, but the lifecycle-manager's periodic-poll path (`runPointerPollOnce`) and cold-start recovery path (`recoverFromAggregatorPointerBestEffort`) still treat the recovered CID as a UXF *bundle* CID — they call `bundleIndex.addBundle(recoveredCid, …)` directly without first parsing the CAR as a lean snapshot. The result is a stale bundle-index entry pointing at snapshot bytes; the next `load()` will then try to parse the snapshot CAR as a UXF package and fail. The bug is latent because the publish-side reconcile loop (where the `fetchAndJoin` path runs) covers most paths in practice. The clean fix is to route the polled CID through the same `applySnapshot` callback (or call `pointer.reconcile`-equivalent). Tracking as a follow-up because the surface needs design — the applier lives on `ProfileStorageProvider` whereas the poll lives on the `LifecycleManager` host, so the fix needs a new host method (`applySnapshotIfWired(cid)?`) symmetric to `publishSnapshotIfWired()`.
