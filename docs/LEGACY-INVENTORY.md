...
> **HISTORICAL snapshot (0.15.x era) — a design record, not a description of the current code.**
> Do not act on an entry without re-verifying it against the tree. Since the snapshot, 0.16.0
> made mainnet a runnable network and removed every v1 pointer and the `dev` network, so several
> entries below name symbols that no longer exist: `DEFAULT_AGGREGATOR_URL`, `DEV_AGGREGATOR_URL`
> and `TEST_AGGREGATOR_URL`; `getEmbeddedTrustBase`'s `case 'dev'` and `TRUSTBASE_DEV`; and
> `RegistryNetwork`'s `'dev'` member (it is now `'testnet' | 'testnet2' | 'mainnet'`). Files the
> REMOVE-NOW lists still name, such as `modules/payments/`, `modules/accounting/`, `modules/swap/`
> and `impl/shared/wallet-api/WalletApiTokenStorageProvider.ts`, were deleted by the P11 flip.
...
**Anchored on SYMBOLS, not coordinates.** The first version carried file:line
references and 93 of them were stale within a day — #722 deleted whole files and
#723 split PaymentsModule into five, so line numbers and even paths moved. Find an
entry by grepping its backticked symbol, not by its address.

**Already landed** (do not re-hunt these):
- #722 — wallet.dat + legacy-file import, the v1 TXF token codec, `core/bech32.ts`,
  `PROXY://`, the `_outbox`/`_sent`/`_invalid` storage slots, the v1 entry sniffer,
  the archived/forked token surface.
- #723 — `mergeTombstones`, `resolveSenderInfo`, the `PENDING_V5_TOKENS` sweep,
  `MAX_SKIP_COUNT`, `RESERVATION_TIMEOUT_MS` + `TokenReservationLedger.cleanup`,
...
`inventory/`, `resume/` or `history/` after the #723 split — the symbol is the
reliable anchor.
...
- where an old shape can still ARRIVE at runtime (a stored record, a wire payload, a
  ...
  But do NOT leave a refusal branch that only buys a nicer message than the
  fall-through already gives — that is legacy code wearing a different hat.

## KEEP — labelled legacy, but load-bearing
These read as legacy and are not. Do not delete them on the strength of a comment.
- **modules/payments/receive/Delivery.ts** — The sphere-envelope (`try`) arm of `handleV2Transfer`'s tolerant blob decode — `try { blob = decodeTokenBlob(bytes) } catch { blob = { v, network: 0, tokenId: '', token: bytes } }`.
...
- **modules/payments/resume/IntentResume.ts,5337-5352** — `interface IntentPayloadV1` (name says V1, only legal `v` is `2`) plus the `if (payload.v !== 2 || !Array.isArray(payload.direct))` refusal in `resumeOpenIntents`.
...
- **core/encryption.ts** — The `EncryptedData` apparatus — `encrypt`, `decrypt`, `decryptJson`, `isEncryptedData`, `serializeEncrypted`, `deserializeEncrypted`, `generateRandomKey`, plus the `encryptMnemonic`/`decryptMnemonic` 
...
- **core/Sphere.ts (declaration), core/Sphere.ts (the `init()` that drops it), core/Sphere.ts/1084/1191 (create/load/import assign it), core/Sphere.ts (buildTokenEngine consumes it), token-engine/factory.ts, docs/VERIFICATION-WORKERS.md, CHANGELOG.md:45-56, tests/integration/address-switch-engine-lifecycle.test.ts** — `SphereInitOptions.verification` (opt-in parallel token-verification worker-pool config). The candidate's factual claim is correct — `Sphere.init()` never forwards it — but the field is NOT legacy: it
  - why it stays: WHAT I TRIED TO FIND THAT WOULD MAKE REMOVAL SAFE — and failed to find:

1. Evidence it is dead everywhere, not just at `init`. REFUTED. `_verification` is read at core/Sphere.ts by `buildTokenEngine`, which runs for EVERY engine (boot, each address switch, `setOracleApiKey` rebuild), and flows
- **types/txf.ts** — The v1 TXF token structure — TxfToken/TxfGenesis/TxfGenesisData/TxfState/TxfTransaction/TxfInclusionProof/TxfAuthenticator/TxfMerkleTreePath/TxfMerkleStep/TxfIntegrity
  - why it stays: I set out to find a producer, a live consumer, or a money path, and found all three.

REFUTATION 1 (decisive — a v2 producer, today). `AccountingModule._scanTokenForAttribution` (modules/accounting/AccountingModule.ts) CONSTRUCTS a TxfToken from a **v2 engine token** and feeds it to the at
- **core/Sphere.ts** — The `fromLegacy` flag inside `syncIdentityWithTransport()` — the "Old-format events don't have content.nametag (only encrypted_nametag)" fallback that calls `transport.recoverNametag()`, then re-publi
...
(1) LIVE CALLER, OUTSIDE THIS REPO. The reader is not dormant. In the actual Sphere frontend (/home/pavelg/unicity/sphere): src/components/wallet/onboarding/hooks/useOnboardingFlow.ts calls Sphere.detectLegacyFile
...
- **token-engine/token-blob.ts** — `TOKEN_BLOB_VERSION = 1` and the `TokenBlob.v` field — the sphere-private CBOR envelope's version discriminator, validated by `decodeTokenBlob` (token-blob.ts).
...
- **token-engine/token-blob.ts** — `unwrapTokenBlobBytes` (tolerant dual-form: decode the sphere-private 39051 `TokenBlob` envelope, else pass bytes through) plus its two mirroring tolerant decodes — `WalletApiTokenStorageProvider.wrap
  - why it stays: I tried four ways to kill it and all four failed.

(1) "The pass-through arm is dead — everything is the envelope." REFUTED. `WalletApiMailboxProvider.toIncomingDelivery().fetchBlob()` (WalletApiMailboxProvider.ts) returns whatever the backend blob store serves, and `WalletApiTokenStoragePro
- **token-engine/engine.ts** — `EngineOpOptions.checkpointStore` being optional, plus the store-less burn branch in `SphereTokenEngine.resolveBurntToken` (/home/pavelg/unicity/sphere-sdk/token-engine/SphereTokenEngine.ts: `
  - why it stays: I set out to prove the candidate's claimed reachability, and it collapsed.

REFUTED CLAIM: "Reachable on every local (non-wallet-api) split today ... the default SDK composition." False. `PaymentsModule.sendOnce()` (/home/pavelg/unicity/sphere-sdk/modules/payments/PaymentsModule.ts) calls
- **modules/swap/dm-protocol.ts** — `parseInvoiceDelivery` requires `invoice_token` to be a non-null OBJECT (v1 TXF JSON) while `AccountingModule.importInvoice` requires a STRING (v2 hex blob) and throws on anything else — the escrow de
  - why it stays: WHAT I CONFIRMED (all candidate facts hold). dm-protocol.ts rejects anything non-object; AccountingModule.ts throws INVOICE_INVALID_DATA on anything non-string. I proved the mutual exclusion by running both shapes through the real `parseSwapDM`: a v2 hex string returns NULL (dropped),
- **modules/accounting/AccountingModule.ts** — `resolveInvoiceRef` (:3581) direct-match branch + the `invoiceIdHashIndex` / `_rebuildHashIndex` (:3556) / `_addToHashIndex` (:3567) machinery.
  - why it stays: I tried to confirm the claim "the hash branch needs an on-chain memo, which nothing writes" and REFUTED it. buildInvoiceMemo (memo.ts) returns `INV:${hashInvoiceId(id)}:${dir}` and is the TRANSPORT memo builder, not the on-chain one — #708 removed parseInvoiceMemoForOnChain (the on-chain produce
- **connect/protocol.ts** — `SphereHandshake.warning?: SphereRpcError` — an optional, non-fatal deprecation-notice slot on the handshake RESPONSE frame. Producer side: `ConnectHost.sendHandshakeResponse()` 6th positional param (
...
- **constants.ts** — NOSTR_EVENT_KINDS.TOKEN_TRANSFER (31113), PAYMENT_REQUEST (31115), PAYMENT_REQUEST_RESPONSE (31116) — and, unavoidably, the whole orphaned Nostr rail hanging off them (transport se
- **core/Sphere.ts** — `Sphere.clear()`'s dual signature: a bare positional `StorageProvider` (legacy) vs the `{ storage, tokenStorage? }` options object, discriminated at runtime by `'get' in storageOrO
- **core/Sphere.ts** — `Sphere.importFromLegacyFile` / `detectLegacyFileType` / `isLegacyFileEncrypted`. PARTLY DONE in #722: the Bitcoin Core `.dat` path is gone; the `.txt` / legacy-JSON / mnemonic restore stays because sphere's onboarding calls all three and `exportToTxt` still writes those backups. Removing the rest means removing the writer too, as one decision.
- **core/Sphere.ts, :4455, :4469, :1621-1622** — Four text fragments (one doc comment, two warn strings, one doc comment) claiming that modules "fall back to their legacy path" / "keep their legacy path" when `buildTokenEngine` r
- **core/Sphere.ts** — `decrypt()` fallback that decrypts a stored mnemonic/master key with the hardcoded `DEFAULT_ENCRYPTION_KEY` ('sphere-default-key') when no password is set and the blob isn't alread
- **core/Sphere.ts** — `Sphere.decrypt()` back-compat branch: with no password and after the plaintext (BIP39 / 64-hex) check fails, decrypt the stored mnemonic / master key with the hardcoded `DEFAULT_E
- **impl/browser/storage/IndexedDBTokenStorageProvider.ts (and impl/nodejs/storage/FileTokenStorageProvider.ts)** — Legacy key-shape handling in the two local token-storage providers: the `token-`/`nametag-` skip filters, the `archived-` load/save branches, and the nodejs `archived_` readdir fil
- **impl/shared/wallet-api/WalletApiMailboxProvider.ts** — `depositChunk`'s 404 arm: a `NOT_FOUND` from `POST /v1/mailbox/batch` is read as "pre-#111 deployment", latches `batchDepositUnsupported = true` on the instance, and permanently de
- **impl/shared/wallet-api/WalletApiTokenStorageProvider.ts** — `mayHaveSpent`'s bare-`${tokenId}` knownSpends read (the declaration: `if (known.has(tokenId)) return true; // legacy bare entry`), documented at the declaration as a shape only "legacy builds w
- **impl/shared/wallet-api/WalletApiTokenStorageProvider.ts** — `wrapWireBlob(tokenId, bytes)` tries `decodeTokenBlob(bytes)` (sphere-private CBOR tag 39051 envelope) before falling back to a raw wrap `{v:1, network:0, tokenId, token: bytes}`. 
- **modules/accounting/AccountingModule.ts** — `_doLoad()` step 2–3: the only reload path for `invoiceTermsCache`, implemented as `JSON.parse(token.sdkData) as TxfToken` + filter on `txf.genesis.data.tokenType === INVOICE_TOKEN
- **modules/accounting/AccountingModule.ts** — The on-chain memo attribution subsystem: `_scanTokenForAttribution` → `_processTokenTransactions` → `decodeTransferMessage` → `tokenScanState` watermarks + `_gapFillTokenScan` + `_
- **modules/payments/PaymentsModule.ts** — `IntentPayloadV1.spentStates?` (optional, "Absent on legacy (pre-M7) payloads", the declaration) plus its two consumer fallbacks: `payload.spentStates?.[id]?.protocol ?? ''` at the `applyInve
- **modules/payments/PaymentsModule.ts** — `isLegacyAlpha` — the `_meta.address?.startsWith('alpha1')` exemption in PaymentsModule.load()'s token-storage address guard, which lets a record whose `_meta.address` is a pre-L1-
- **modules/payments/PaymentsModule.ts** — The `isLegacyAlpha` escape hatch in PaymentsModule.load()'s token-storage address guard: `_meta.address?.startsWith('alpha1')` suppresses the cross-address rejection for pre-L1-rem
- **modules/payments/PaymentsModule.ts** — `acceptPaymentRequest(requestId)` (the declaration-2768) and `markPaymentRequestPaid(requestId)` (the declaration-2795) — two public PaymentsModule entry points that only mutate in-memory request stat
- **modules/payments/PaymentsModule.ts** — `ReceiveOptions` (declaration :382, export `index.ts`) and the unread `_options?: ReceiveOptions` first parameter of `PaymentsModule.receive()` (:3148).
- **modules/payments/PaymentsModule.ts** — `isV2TransferPayload` (defined in /home/pavelg/unicity/sphere-sdk/types/v2-transfer.ts) is imported at PaymentsModule.ts and never called. The `type: 'V2_TRANSFER'` / `versio
- **modules/payments/PaymentsModule.ts** — The local minted-token nametag store in PaymentsModule: the `nametags: NametagData[]` field (the declaration), its five public methods `setNametag`/`getNametag`/`getNametags`/`hasNametag`/`cl
- **modules/payments/PaymentsModule.ts** — `payments.sync()` — the post-d1b1c397 shell around `save()`: the `sync` name, the constant `{added: 0, removed: 0}` return, the `sync:started`/`sync:completed`/`sync:error` events,
- **modules/payments/PaymentsModule.ts** — The non-blob fallback branch of `parseSdkDataCached` — `JSON.parse(sdkData)` → `genesis.data.tokenId` + `getCurrentStateHash(txf)`, plus the three further guesses `state.hash` / `s
- **modules/payments/PaymentsModule.ts** — The non-blob arm of `parseSdkDataCached` — JSON.parse of `sdkData` as v1 TXF to recover `(tokenId, stateHash)`, plus the three "alternative location" probes (`state.hash`, `stateHa
- **modules/payments/PaymentsModule.ts** — `PaymentsModuleConfig` (all five options: `autoSync`, `autoValidate`, `retryFailed`, `maxRetries`, `debug`), the private `moduleConfig` field (:947), the constructor defaulting blo
- **modules/payments/PaymentsModule.ts** — `PaymentsModuleDependencies.tokenStorage` — the `@deprecated` single-provider alias for `tokenStorageProviders`, plus its two fallback branches: `firstEnabledTokenStorage` (the declaration-89
- **modules/swap/dm-protocol.ts,512)** — Every protocol-v1 inbound branch in the swap module: the `!swap.protocolVersion || swap.protocolVersion < 2` announce arm (SwapModule.ts), the v1 crash-recovery `else` (:404),
- **oracle/UnicityAggregatorProvider.ts** — The oracle provider's event subsystem: `eventCallbacks` (:57), `onEvent()` (:159-162), `emitEvent()` (:168-176), its two emit call sites in `connect()`/`disconnect()` (:115, :121),
- **serialization/txf-serializer.ts** — Six v1-TXF utility exports: getTokenId, getCurrentStateHash, hasValidTxfData, hasUncommittedTransactions, hasMissingNewStateHash, countCommittedTransactions.
- **token-engine/types.ts** — `TransferParams.data` (types.ts) and `SplitOutput.data` (types.ts) — the on-chain memo INPUT to a transfer/split output — plus their two engine plumbing sites (`par
- **token-engine/types.ts** — `TokenBlob.network` — the second element of the sphere-private CBOR tag-39051 envelope (`tag(39051)[v, network, tokenId, token]`), written by `encodeTokenBlob` and read back by `de
- **transport/MultiAddressTransportMux.ts** — kind-14 control-message compatibility filter (`read_receipt` / `typing` / bare-composing JSON smuggled in a normal DM gift wrap) + the self-wrap suppressor at the declaration
- **transport/websocket.ts)** — The `Math.random()` UUID fallbacks in `createRequestId()` (connect/protocol.ts) and `defaultUUIDGenerator()` (transport/websocket.ts), both guarded by a `crypto.rando
- **types/txf.ts (+ modules/payments/PaymentsModule.ts,4121-4183,6151,6196; serialization/txf-serializer.ts,365-423)** — NametagData, the `_nametag`/`_nametags` storage slots, and the PaymentsModule nametag-token store (private `nametags[]`, setNametag/getNametag/getNametags/hasNametag/clearNametag) 
- **types/txf.ts** — ARCHIVED_PREFIX / FORKED_PREFIX constants plus the six exported key helpers isArchivedKey, isForkedKey, tokenIdFromArchivedKey, archivedKeyFromTokenId, forkedKeyFromTokenIdAndState
- **wallet-api/client.ts** — `WalletApiClient.connectWakeSocket(onWake)` — the bare, non-self-healing wake socket (nulls `onerror`/`onclose` after open) plus its return type `WakeSocketHandle` (wallet-api/type

## REMOVE-NOW

### constants.ts
- Storage keys `PROCESSED_SPLIT_GROUP_IDS` ('processed_split_group_ids') and `PROCESSED_COMBINED_TRANSFER_IDS` ('processed_combined_transfer_ids') — declared in `STORAGE_KEYS_ADDRESS` (constants.ts
...
- STORAGE_KEYS_ADDRESS.PENDING_V5_TOKENS ('pending_v5_tokens'), its NETWORK_SCOPED_ADDRESS_KEYS entry (constants.ts), the delete-on-load block at modules/payments/PaymentsModule.ts, and th
- STORAGE_KEYS_ADDRESS.PROCESSED_SPLIT_GROUP_IDS (V5) and PROCESSED_COMBINED_TRANSFER_IDS (V6) — Nostr re-delivery dedup ledgers — plus their two NETWORK_SCOPED_ADDRESS_KEYS entries (constants.ts
...
- DEFAULT_AGGREGATOR_URL (283), DEV_AGGREGATOR_URL (286), TEST_AGGREGATOR_URL (289) — three v1-era aggregator endpoint constants, re-exported from index.ts, two of which populate NETWORKS.mainne
...
### core/discover.ts
- Single-inhabitant discriminator unions left from two-phase (transport + L1) discovery: `DiscoverAddressProgress.phase: 'transport'` (core/discover.ts, written :126) and `DiscoveredAddress.source
...
- `getEmbeddedTrustBase`'s `case 'dev'` returns `TRUSTBASE_DEV`, a straight alias of `TRUSTBASE_TESTNET` — the v1 networkId-3 trust base (assets/trustbase.ts, aliased at :76 with the comment "same 
...
- `_meta: { version: 1, formatVersion: '2.0' }` stamped by all three token-storage providers' `load()` (WalletApiTokenStorageProvider.ts, IndexedDBTokenStorageProvider.ts, FileTokenStorageProvid
...
- `importInvoice(token: TxfToken | string)` — the public param still admits the v1 TXF object type only so the declaration can throw a bespoke "Legacy (v1 TXF)" rejection for it.
...
- `parseInvoiceMemoForOnChain` (memo.ts) — the on-chain invoice memo builder, plus its helper `encodeTransferMessage` (memo.ts) and the now-orphaned `DIRECTION_TO_CODE` (memo.ts). Both bu
...
- `loadHistory()`'s "One-time migration from legacy KV storage" block (the declaration-3907: read `STORAGE_KEYS_ADDRESS.TRANSACTION_HISTORY`, `importHistoryEntries`, `storage.remove`) plus the terminal `else` arm
...
- The `_history` round-trip through the TXF storage blob: `MAX_SYNCED_HISTORY_ENTRIES` (PaymentsModule.ts), the `historyEntries: sorted.slice(0, 5000)` write in `createStorageData` (:6142-6153), the
- `resolveSenderInfo(senderTransportPubkey)` (PaymentsModule.ts) and its single call in `handleV2Transfer` (:4572), plus the `...senderInfo` spread into `addToHistory` (:4591) — a Nostr transp
- `resolveTransportPubkey(recipient, peerInfo)` (the declaration-4470) and its single call site in `resolveSendTarget` (the declaration) — deriving a Nostr x-only transport pubkey for a send. Its only surviving output is t
- `PendingV2Delivery.opIndex?` (optional, the declaration, "Absent on legacy entries") and the positional fallback `byOp.set(e.opIndex ?? i, e)` in `journaledByOp` (the declaration).
...
- The `&& this.paymentRequestsApi()` conjunct on the #441 settling branch, plus the two comments that justify it as the "Nostr-only composition" fallback (the declaration-2896 rationale and the first clause of th
...
- `buildProposalDM` — the protocol-v1 swap-proposal DM emitter. Exported from `modules/swap/dm-protocol.ts`, re-exported by the internal barrel `modules/swap/index.ts`, imported (unused) at `modules/s
...
- `export type RegistryNetwork = 'testnet' | 'mainnet' | 'dev';` — a type-only alias with zero uses, re-exported at /home/pavelg/unicity/sphere-sdk/registry/index.ts and /home/pavelg/unicity/sphere-s

### serialization/txf-serializer.ts
- The `archivedTokens`/`forkedTokens` options on `buildTxfStorageData` (the declaration, 338-353), the two Map fields on `ParsedStorageData` (367-368, 385-386), and the `isArchivedKey`/`isForkedKey` parse
- The singular `_nametag` back-compat reader in `parseTxfStorageData()` (the declaration), plus the `seenNames` set (the declaration) that exists only to serve it.
...
- `ITokenEngine.deriveIdentityAddress(pubkey?)` (port declaration) + `SphereTokenEngine.deriveIdentityAddress` (/home/pavelg/unicity/sphere-sdk/token-engine/SphereTokenEngine.ts) — a port method
- `ITokenEngine.readMemo` (token-engine/engine.ts) + its implementation `SphereTokenEngine.readMemo` (token-engine/SphereTokenEngine.ts) — the reader for the on-chain memo channel. Sole prod

### token-engine/identity.ts
- The `export` modifier on `UNICITY_TOKEN_TYPE_HEX` and the stale half of its doc block that still points at the deleted `token-engine/unicity-id.ts`. The `const`, its value, an

### token-engine/network.ts
- `toNetworkId()` (whole file `token-engine/network.ts`), the `SphereNetwork` union it consumes (`token-engine/types.ts`), the `SphereNetwork` re-export from the public `./token-engine` barrel (`t
...
- `NostrTransportProvider.sendTypingIndicator` (the declaration) emits a kind-14 NIP-17 gift wrap carrying `{type:'typing', senderNametag}`, plus its matching kind-14 receive branch at the declaration. Bo
...
- `OutboxEntry`, `MintOutboxEntry`, `InvalidatedNametagEntry` (types/txf.ts) plus their `_outbox` / `_mintOutbox` / `_invalidatedNametags` legs in `serialization/txf-serializer.ts` (build option
...
- `ChallengeExpectation.nowMs` — publicly exported, explicitly `@deprecated` optional field that `verifyChallengeTemplate` never reads; `WalletApiClient.challengeSignIn` (client.ts) still passes `no
