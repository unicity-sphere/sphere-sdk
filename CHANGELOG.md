...
### Changed
- **Types — `DEFAULT_IPFS_GATEWAYS` widened to `readonly string[]`** (previously `readonly ['https://unicity-ipfs1.dyndns.org']` literal tuple). Required to accommodate the new `SPHERE_IPFS_GATEWAY` env override. No runtime behavior change. Consumers relying on the literal-tuple type should switch to `BUILTIN_IPFS_GATEWAYS` (which retains the `as const` shape).
