...
- **UXF Inter-Wallet Transfer Protocol — spec-level changes** (implementation lands in subsequent waves; see `docs/uxf/UXF-TRANSFER-PROTOCOL.md`):
  ...
  - **Most-recent-proof rule** — same `requestId` + same value can have multiple valid proofs across BFT rounds; canonicalize by latest BFT round (replaces lex-min CID for proof canonicalization). Two proofs for same `requestId` with different values → `transfer:security-alert` (single-spend violation; out-of-scope hostile path). See UXF-TRANSFER-PROTOCOL §6.3.
