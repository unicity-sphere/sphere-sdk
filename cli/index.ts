...
SHELL COMPLETION:
  completions <shell>               Generate completion script (bash/zsh/fish)
                                    Setup: eval "$(sphere-cli completions bash)"
...
function getCompletionCommands(): CompletionCommand[] {
  return [
    // Wallet management
    { name: 'init', description: 'Create or import wallet', flags: ['--network', '--mnemonic', '--nametag', '--no-nostr'] },
    ...
    // Market
    { name: 'market-post', description: 'Post an intent', flags: ['--type', '--category', '--price', '--currency', '--location', '--contact', '--expires'] },
    { name: 'market-search', description: 'Search intents', flags: ['--type', '--category', '--min-price', '--max-price', '--min-score', '--location', '--limit'] },
    { name: 'market-my', description: 'List your own intents' },
    { name: 'market-close', description: 'Close an intent' },
    { name: 'market-feed', description: 'Watch the live listing feed', flags: ['--rest'] },
    // Invoices
    ...
    // Daemon
    { name: 'daemon', description: 'Manage event listener daemon', subcommands: [
      { name: 'start', description: 'Start persistent event listener', flags: ['--config', '--detach', '--log', '--pid', '--event', '--action', '--market-feed', '--verbose'] },
      { name: 'stop', description: 'Stop running daemon' },
      { name: 'status', description: 'Check if daemon is running' },
    ]},
    // Encryption
    { name: 'encrypt', description: 'Encrypt data with password' },
    { name: 'decrypt', description: 'Decrypt encrypted JSON data' },
    // Wallet parsing
    ...
    if (cmd.subcommands) {
      ...
      for (const sub of cmd.subcommands) {
        if (sub.flags && sub.flags.length > 0) {
          flagCases.push(`    ${cmd.name})\n      case "\${words[2]}" in\n        ${sub.name})\n          flags="${sub.flags.join(' ')}"\n          ;;\n      esac\n      ;;`);
        }
      }
    }
    if (cmd.flags && cmd.flags.length > 0 && !cmd.subcommands) {
...
# Also complete for npx tsx cli/index.ts usage
complete -F _sphere_cli tsx
`;
}
...
function generateZshCompletions(): string {
  ...
  const commandDescriptions = cmds.map(c =>
    `    '${c.name}:${c.description.replace(/'/g, "'\\''")}'`
  ).join('\n');
  ...
  for (const cmd of cmds) {
    if (cmd.subcommands) {
      const subDescs = cmd.subcommands.map(s =>
        `          '${s.name}:${s.description.replace(/'/g, "'\\''")}'`
      ).join('\n');
      const subArgs = cmd.subcommands.map(s => {
        if (!s.flags || s.flags.length === 0) return '';
        return `          ${s.name})\n            _arguments \\\n${s.flags.map(f => `              '${f}[${f} option]'`).join(' \\\n')}\n            ;;`;
      }).filter(Boolean).join('\n');
      subcmdCases.push(`      ${cmd.name})\n        local -a subcmds\n        subcmds=(\n${subDescs}\n        )\n        if (( CURRENT == 3 )); then\n          _describe 'subcommand' subcmds\n        else\n          case "\${words[3]}" in\n${subArgs}\n          esac\n        fi\n        ;;`);
    } else if (cmd.flags && cmd.flags.length > 0) {
      subcmdCases.push(`      ${cmd.name})\n        _arguments \\\n${cmd.flags.map(f => `          '${f}[${f} option]'`).join(' \\\n')}\n        ;;`);
    }
  }
...
#   # or
#   sphere-cli completions zsh >> ~/.zshrc
...
function generateFishCompletions(): string {
  ...
  for (const cmd of cmds) {
    if (cmd.subcommands) {
      ...
      // Subcommands
      for (const sub of cmd.subcommands) {
        lines.push(`complete -c sphere-cli -n '__fish_seen_subcommand_from ${cmd.name}; and not __fish_seen_subcommand_from ${cmd.subcommands.map(s => s.name).join(' ')}' -a '${sub.name}' -d '${sub.description}'`);
        if (sub.flags) {
          for (const flag of sub.flags) {
            const flagName = flag.replace(/^--/, '');
            lines.push(`complete -c sphere-cli -n '__fish_seen_subcommand_from ${cmd.name}; and __fish_seen_subcommand_from ${sub.name}' -l '${flagName}' -d '${flagName} option'`);
          }
        }
      }
    } else {
      ...
      if (cmd.flags) {
        for (const flag of cmd.flags) {
          if (flag.startsWith('--')) {
            const flagName = flag.replace(/^--/, '');
            lines.push(`complete -c sphere-cli -n '__fish_seen_subcommand_from ${cmd.name}' -l '${flagName}' -d '${flagName} option'`);
          } else if (flag.startsWith('-') && flag.length === 2) {
            lines.push(`complete -c sphere-cli -n '__fish_seen_subcommand_from ${cmd.name}' -s '${flag.charAt(1)}' -d '${flag} option'`);
          }
        }
      }
    }
